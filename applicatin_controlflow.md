...
B -- Submits w/ Photo --> C[POST /api/report (w/ Auth Token)];
C --> D[Backend: report.py];
D -- Gets user_id --> E[Generate Embedding (recognition.py)];
E --> F[Save to parents/volunteers.json];
...
J --> K[ReportForm: Show "Submitted" Message"];
I -- Yes --> L[Update both DB records w/ unconfirmed match];
...
C -- Finds both reports --> D[Return Parent & Volunteer Data];
D --> E[Page Displays Both Images & Details];
E -- User Clicks "Yes" --> F[POST /api/confirm];
F --> G[Backend: match.py];
G -- confirmed:true --> H[Create entry in children.json];
H --> I[Return OK];
